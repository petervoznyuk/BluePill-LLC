...
~/.platformio/penv/Scripts/platformio.exe run --target upload --target monitor --environment bluepill_f103c8 --monitor-port COM5 > data/2024-03-06/$(date +%Y%m%d)_time$(date +%H%M%S).csv